...
__global__ void conv1DBlocs(const int N, const float *x, float *y) {
  int idx = blockIdx.x;
  if (idx < N) {
    if (idx == 0 || idx == N - 1) {
      y[idx] = x[idx];
...
__global__ void conv1DBlocsThreads(const int N, const float *x, float *y) {
...
// Calcul de convolution 1D en utilisant blockSize thread par bloc et effectuant
// k operation par thread dans un bloc
__global__ void conv1DBlocsThreadsKops(const int N, const float *x, float *y,
                                       const int k) {
  int first = (blockIdx.x * blockDim.x + threadIdx.x) * k;
  for (int j = 0; j < k; j++) {
    int idx = first + j;
    if (idx < N) {
      if (idx == 0 || idx == N - 1) {
...
int main(int argc, char **argv) {
  ...
  // Lancer le kernel conv1DBlocsThreads avec un certain blockSize et nombre de
  // bloc A FAIRE ... blockSize = 32, 64, 128, 256, 512, 1024
  blockSize = 1024;
  ...
  // Lancer le kernel conv1DBlocsThreadsKops avec un certain blockSize, nombre
  // de bloc, et nombre d'operations par thread (variable k) A FAIRE ...
  // blockSize = 32, 64, 128, 256, 512, 1024
  // k = 1, 2, 4, 8, 16, ...
  blockSize = 1024;
  k = 8;
  conv1DBlocsThreadsKops<<<(N + (blockSize * k) - 1) / (blockSize * k),
                           blockSize>>>(N, dx, dy, k);
